...
def workspace(tmp_path: Path) -> Path:
    onnx = tmp_path / "models" / "voice_asr" / "artifacts" / "torch" / "silero-vad" / "silero_vad.onnx"
    om = tmp_path / "models" / "voice_asr" / "artifacts" / "ascend" / "silero_vad" / "silero_vad_v6_310p_mixed16.om"
...
def test_bundle_contains_both_deployments(tmp_path: Path, workspace: Path) -> None:
    ...
    assert DEFAULT_ONNX_REL in files
